/* globals  ko, iAjax, iLang, INSTAPAGE_AJAXURL, masterModel */
...
var EditModel = function EditModel(data) {
  ...
  self.id = data && data.page ? ko.observable(data.page.id) : ko.observable(0);
  self.instapageId = data && data.page ? data.page.instapage_id : null;

  if (data && data.subAccounts && data.subAccounts.length) {
    self.subAccounts = ko.observableArray(data.subAccounts);
    self.choosenSubAccount = data.page ? ko.observable(data.page.subaccount) : ko.observable(data.subAccounts[0].id);
  } else {
    self.subAccounts = ko.observableArray();
    self.choosenSubAccount = ko.observable();
    masterModel.messagesModel.addMessage(iLang.get('LOGIN_OR_ADD_TOKEN_FIRST'));
  }

  self.choosenLandingPage = data && data.page ? ko.observable(data.page.instapage_id) : ko.observable();
  self.choosenLandingPageType = data && data.page ? ko.observable(data.page.type) : ko.observable();
  self.slug = data && data.page ? ko.observable(data.page.slug) : ko.observable('');
  self.isSlugValid = ko.observable(true);
  self.isTypeValid = ko.observable(true);
  self.landingPages = ko.observableArray();
  self.pageTypes = ko.observableArray([new LandingPageType('page', 'Landing Page'), new LandingPageType('home', 'Home Page'), new LandingPageType('404', '404 Page')]);
